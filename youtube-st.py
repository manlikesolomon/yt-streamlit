...
def load_data():
    ...
    df_comm = pd.read_csv('data/All_Comments_Final.csv')
    ...
    return df_agg, df_agg_sub, df_comm, df_time

df_agg, df_agg_sub, df_comm, df_time = load_data()


# data engineering
df_agg_diff = df_agg.copy()
metric_date_12mo = df_agg_diff['Video publish time'].max() - pd.DateOffset(months=12)
numeric_columns = df_agg_diff.select_dtypes(include=np.number).columns.tolist()
...
# build dashboard
add_sidebar = st.sidebar.selectbox('Aggregate Metric or Individual Video', ['Aggregate', 'Individual'])
    ...
    for i in metric_medians6mo.index:
        if i != 'Video publish time':
            with columns[count]:
                ...
                st.metric(label=i, value= round(metric_medians6mo[i],3), delta=str(round(delta,2)) + "%")
                count += 1
                if count >= 5:
                    count = 0
    df_agg_diff['Publish_date'] = df_agg_diff['Video publish time'].apply(lambda x: x.date())
    df_agg_final = df_agg_diff[['Video title','Publish_date','Views','Likes','Subscribers','Shares','Comments added','RPM(USD)','Average % viewed',
                             'Avg_duration_sec', 'Engagement_ratio','Views / sub gained']]
    st.dataframe(df_agg_final.sort_values(by='Views', ascending=False))
...
if add_sidebar == 'Individual':
    ...
    agg_sub_filtered['Country'] = agg_sub_filtered['Country Code'].apply(audince_sample)
    ...
    # show total views
    video_views = agg_filtered['Views']
    st.metric('Total Views', video_views)
    ...
    fig2.update_layout(title='View Comparism (30 days)',
